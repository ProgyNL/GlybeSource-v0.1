...
if(!isset($user))
{
	DB::Query("DELETE FROM `users_online` WHERE `ip` = '" . $_SERVER['REMOTE_ADDR'] . "' AND `u_a` = '" . $_SERVER['HTTP_USER_AGENT'] . "'");
	DB::Query("INSERT INTO `users_online` (user_id, u_a, ip, last_active, last_page) VALUES (0, '" . $_SERVER['HTTP_USER_AGENT'] . "', '" . $_SERVER['REMOTE_ADDR'] . "', CURRENT_TIMESTAMP(), '" . DB::Escape($_SERVER['REQUEST_URI']) . "')");
}
if(isset($user))
{
