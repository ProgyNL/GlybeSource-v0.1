...
$version = "Glybe Vrijgegeven 0.1";

header("Content-Type: text/html; charset=utf-8");

if($_SERVER['HTTP_HOST'] != "www.glybe.scripts.hetisniels.nl")
{
	header("HTTP/1.1 301 Moved Permanently");
	header("Location: http://www.glybe.scripts.hetisniels.nl" . $_SERVER['REQUEST_URI']);
	die();
}
...
function checkIfIsNlOrBe($ip)
{
	...
	{
		if(stristr($gethostbyaddr, "nuspelen.nl"))
			return false;
		return true;
	}
	else
...
$glb['db'] = Array();
$glb['db']['gebruiker'] = "gerlifl84_glb";
$glb['db']['wachtwoord'] = "s8x3qEER";
$glb['db']['host'] = "localhost";
$glb['db']['db'] = "gerlifl84_glb";
...
function jesseRound($n,$x=5) // Faalscript van Jesse, Word niet meer gebruikt?
{
	return $x * round($n / $x);
}
?>